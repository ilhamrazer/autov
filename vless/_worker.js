...
async function handleCallbackQuery(callbackQuery) {
  ...
  const afrcloud = FAKE_HOSTNAME; // Menggunakan afrcloud sebagai default host
...
async function handleShadowSocksCreation(chatId, ip, port, isp, afrcloud) {
  const ssTls = `ss://${btoa(`none:${crypto.randomUUID()}`)}@${afrcloud}:443?encryption=none&type=ws&host=${afrcloud}&path=%2F${ip}%3D${port}&security=tls&sni=${afrcloud}#${isp}`;
